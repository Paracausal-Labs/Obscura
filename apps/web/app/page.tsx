...
function FeatureCard({ number, text1, text2, glowPosition, rotate = "none" }: { number: string; text1: string; text2: string; glowPosition: string; rotate?: "left" | "right" | "none" }) {
  let gradientClass = "bg-gradient-to-br from-[#ff0033]/10 to-transparent";
  let blurClass = "absolute -bottom-10 -right-10 w-40 h-40 bg-[#ff0033] rounded-full blur-[60px] opacity-20";

  if (glowPosition === "bottom") {
    gradientClass = "bg-gradient-to-t from-[#ff0033]/20 to-transparent";
    blurClass = "absolute -bottom-16 left-1/2 -translate-x-1/2 w-48 h-32 bg-[#ff0033] rounded-full blur-[60px] opacity-30";
  } else if (glowPosition === "bottom-right") {
    gradientClass = "bg-gradient-to-tl from-[#ff0033]/20 via-transparent to-transparent";
    blurClass = "absolute -bottom-12 -right-8 w-48 h-48 bg-[#ff0033] rounded-full blur-[70px] opacity-30";
  } else if (glowPosition === "left") {
    gradientClass = "bg-gradient-to-r from-[#ff0033]/20 to-transparent";
    blurClass = "absolute top-1/2 -left-16 -translate-y-1/2 w-40 h-48 bg-[#ff0033] rounded-full blur-[60px] opacity-25";
  } else if (glowPosition === "top-left") {
    gradientClass = "bg-gradient-to-br from-[#ff0033]/15 to-transparent";
    blurClass = "absolute -top-10 -left-10 w-40 h-40 bg-[#ff0033] rounded-full blur-[60px] opacity-20";
  }

  let transformStyle = "none";
  if (rotate === "left") transformStyle = "perspective(1000px) rotateY(-12deg)";
  if (rotate === "right") transformStyle = "perspective(1000px) rotateY(12deg)";
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 z-0 ${gradientClass} opacity-80 mix-blend-screen transition-opacity duration-500 group-hover:opacity-100`}></div>
      <div className={`${blurClass} transition-opacity duration-500 group-hover:opacity-60`}></div>

      <div className="relative z-10 flex flex-col justify-start space-y-6">
        <div className="text-[#ff1a40] font-semibold text-[22px] tracking-wide">{number}</div>

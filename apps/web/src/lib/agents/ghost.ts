  ...
  protected async execute(
    ...
    if (!isBitGoEnabled()) {
      ...
      try {
        ...
        const markdown = `# Ghost Quote Preview — Job #${jobId}\n\n${buildPreviewReport(swapParams, quote)}\n\n---\n*Generated by Obscura at ${new Date().toISOString()}*`;
    ...
    try {
      ...
      reportHash = report.encryptedContent;
      reportFileId = report.fileId;
    } catch {
      const key = deriveKeyFromSignature(userSignature, job.id.toString());
      const markdown = `# Ghost Execution — Job #${jobId}\n\n${reportContent}\n\n---\n*Generated by Obscura at ${new Date().toISOString()}*`;
      const payload = encrypt(markdown, key);
      reportHash = encodePayload(payload);
      reportFileId = `local:${job.id}`;
      storeLocalReport(job.id.toString(), reportHash);
    }
    ...
    return {
      ...
      deliverableHash: signingFailed ? "" : reportHash,
      fileverseFileId: reportFileId,
      metadata: {
        toolsCalled,
        duration: 0,
